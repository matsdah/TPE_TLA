...
declarationList
	: %empty										{ $$ = EmptyDeclarationListSemanticAction(); }
	| declarationList declaration					{ $$ = AppendDeclarationSemanticAction($1, $2); }
	;

declaration
	: CHARACTER STRING AS IDENTIFIER COLOR STRING SEMICOLON
													{ $$ = CharacterDeclarationSemanticAction($2, $4, $6); free($2); free($4); free($6); }
	...
	| SCENE STRING OPEN_BRACE statementList CLOSE_BRACE
													{ $$ = SceneDeclarationSemanticAction($2, $4); free($2); }
	;
...
dialogueStatement
	: IDENTIFIER STRING								{ $$ = DialogueStatementSemanticAction($1, $2); free($1); free($2); }
	;

showStatement
	: SHOW BACKGROUND IDENTIFIER SEMICOLON			{ $$ = ShowStatementSemanticAction(DISPLAY_BACKGROUND, $3); free($3); }
	| SHOW SPRITE IDENTIFIER SEMICOLON				{ $$ = ShowStatementSemanticAction(DISPLAY_SPRITE, $3); free($3); }
	;

hideStatement
	: HIDE BACKGROUND IDENTIFIER SEMICOLON			{ $$ = HideStatementSemanticAction(DISPLAY_BACKGROUND, $3); free($3); }
	| HIDE SPRITE IDENTIFIER SEMICOLON				{ $$ = HideStatementSemanticAction(DISPLAY_SPRITE, $3); free($3); }
	;

playStatement
	: PLAY MUSIC IDENTIFIER SEMICOLON				{ $$ = PlayStatementSemanticAction(AUDIO_MUSIC, $3); free($3); }
	| PLAY SOUND IDENTIFIER SEMICOLON				{ $$ = PlayStatementSemanticAction(AUDIO_SOUND, $3); free($3); }
	;

stopStatement
	: STOP MUSIC IDENTIFIER SEMICOLON				{ $$ = StopStatementSemanticAction(AUDIO_MUSIC, $3); free($3); }
	| STOP SOUND IDENTIFIER SEMICOLON				{ $$ = StopStatementSemanticAction(AUDIO_SOUND, $3); free($3); }
	;

gotoStatement
	: GOTO STRING SEMICOLON							{ $$ = GotoStatementSemanticAction($2); free($2); }
	;

setStatement
	: SET IDENTIFIER ASSIGN expression SEMICOLON	{ $$ = SetStatementSemanticAction($2, ASSIGN_OP, $4); free($2); }
...
choiceStatement
	: CHOICE OPEN_BRACE optionList CLOSE_BRACE		{ $$ = ChoiceStatementSemanticAction($3); }
	;
...
option
	: STRING OPEN_BRACE statementList CLOSE_BRACE	{ $$ = ChoiceOptionSemanticAction($1, $3); free($1); }
	;

ifStatement
	: IF OPEN_PARENTHESIS condition CLOSE_PARENTHESIS OPEN_BRACE statementList CLOSE_BRACE
													{ $$ = IfStatementSemanticAction($3, $6, NULL); }
	| IF OPEN_PARENTHESIS condition CLOSE_PARENTHESIS OPEN_BRACE statementList CLOSE_BRACE
	  ELSE OPEN_BRACE statementList CLOSE_BRACE
													{ $$ = IfStatementSemanticAction($3, $6, $10); }
	;

endStatement
	: END SEMICOLON									{ $$ = EndStatementSemanticAction(); }
	;

condition
	: expression[l] LESS expression[r]				{ $$ = ConditionSemanticAction($l, CMP_LT, $r); }
...
expression
	: expression[l] ADD expression[r]				{ $$ = BinaryExpressionSemanticAction($l, ARITH_ADD, $r); }
